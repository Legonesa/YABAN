...
TreeActorModel::TreeActorModel(const char* DataSetName, int SizeOfData, int data_batch, TransformerType type) {
...
void TreeActorModel::TokenizeData(){
...
void TreeActorModel::Embedding(int embedSize, int epoch, long double learningRate, int attentionSize){
    EmbedSize = embedSize;
    ...
    while (true) {
        ...
        if (readRow + batch_size < DataSize) {
            ...
            for (int b = (readRow + 1) - batch_size; b <= readRow; b++) {
                ...
                case StandartLanguageTransformer:
                    ...
                    for (int i = 0; i < word.length(); i++) {
                        if (word[i] != ' ' && word[i] != ',' && word[i] != '!' && word[i] != '?' && word[i] != '.' && word[i] != ';')
                            ...
                            if(flood != "") words.push_back(flood);
                            if (word[i] != ' ') {
                                flood = word[i];
...
std::vector<long double> TreeActorModel::predictEmbed(int wordToken, int vocap) const {
...
void TreeActorModel::Learn(int epoch, long double learningRate, int max_seq, int maxDecoder){
    this->maxDecoder = maxDecoder;
        ...
        while (true){
            if (readRow + batch_size / 2 < DataSize){
                ...
                for (int b = (readRow + 1) - batch_size / 2; b <= readRow; b++) {
                    ...
                    case StandartLanguageTransformer:
                        ...
                        for (int i = 0; i < word.length(); i++) {
                            if (word[i] != ' ' && word[i] != ',' && word[i] != '!' && word[i] != '?' && word[i] != '.' && word[i] != ';')
                                ...
                                if (i == word.length() - 1) EvAlRi = false;
                                if(flood != "") Sentence0.push_back(tokenizedVocap1.at(flood));
                                if (word[i] != ' '){
                                    flood = word[i];
                        ...
                        Sentence0.push_back(tokenizedVocap1.at("<EOS>"));
                        word = Data->getValueStr(Data->getCategoryName(2), b);
                        EvAlRi = true;
                        //İkinci kategorinin değerlerini okur.
                        for (int i = 0; i < word.length(); i++) {
                            if (word[i] != ' ' && word[i] != ',' && word[i] != '!' && word[i] != '?' && word[i] != '.' && word[i] != ';')
                                flood = flood + (char)std::tolower(word[i]);
                            else {
                                if (i == word.length() - 1) EvAlRi = false;
                                if(flood != "") Sentence1.push_back(tokenizedVocap2.at(flood));
                                if (word[i] != ' ') {
                                    flood = word[i];
                        ...
                        Sentence1.push_back(tokenizedVocap2.at("<EOS>"));
                        break;
                    case RegressionTransformer:
                        word = Data->getValueStr(Data->getCategoryName(1), b);
                        flood = "";
                    ...
                    if(transformer_Type == RegressionTransformer){
                    ...
                    while(NextWord != tokenizedVocap2.at("<EOS>") && coutn < max_seq){
                        ...
                        if(NextWord == -1){
                            ...
                            if(transformer_Type == RegressionTransformer) NextValue = 0;
                        }
                ...
                for (int b = readRow + 1; b <= DataSize; b++) {
                    ...
                    while(NextWord != tokenizedVocap2.at("<EOS>") && coutn < max_seq){
                        ...
                        for(int i = 0; i < nextWordVec.size(); i++){
                            if(nextWordVec.at(i) >= max){
                                max = nextWordVec.at(i); NextWord2 = NextWord; NextWord = i;
                            }
                        }
                        delete[] NextWordOutput;
                        ResultSentence.push_back(NextWord);
                        ...
                        std::cout << "Loss: " << cross_entropy_loss(nextWordVec, expected) << std::endl;
                        DecoderLayer(0, NextWord2, max_seq, Sentence0, Sentence1, CategoryValue1, EncoderValues, EncoderKeys);
                    }
                    std::cout << "-------------------------------------------------------------------" << std::endl;
                    /*                                      DECODER_KATMANI_SONU                                           */
                    /*                                       BACKPROPAGATION                                            */
...
void TreeActorModel::DecoderLayer(int count, int NextWord, int max_seq, std::vector<int> Sentence0, std::vector<int> Sentence1, std::map<int, long double> CategoryValue1, long double ***EncoderValues, long double ***EncoderKeys){
    if(maxDecoder > count){
        ...
        std::vector<long double**> DecoderKeys;
        std::map<int, std::vector<long double>> EmbedResults1;
        long double NextValue = 0, NextWord2 = 0;
        std::vector<int> ResultSentence;
        std::map<int, long double> ResultValues;
        std::vector<std::vector<long double>> LossValues;
        std::vector<long double> RegressionErr;
        ...
        while(NextWord != tokenizedVocap2.at("<EOS>") && coutn < max_seq){
            ...
            std::cout << "Loss: " << cross_entropy_loss(nextWordVec, expected) << std::endl;
            DecoderLayer(count, NextWord2, max_seq, Sentence0, Sentence1, CategoryValue1, EncoderValues, EncoderKeys);
        }
        std::cout << "-------------------------------------------------------------------" << std::endl;
    }
}
...
std::vector<long double> TreeActorModel::softMax(int outputLenght, long double* Outputs) const {
...
std::vector<long double> TreeActorModel::CrossEntropyLoss(std::vector<long double>& softMaxOutputs, std::vector<long double>& ExpectedValues) const {
...
long double TreeActorModel::cross_entropy_loss(const std::vector<long double>& y_pred, const std::vector<long double>& y_true) const {
...
long double TreeActorModel::sigmoid(long double i) const {
    return 1 / (1 + exp((float)-i));
}
long double TreeActorModel::dSigmoid(long double i) const {
    return (1 / (1 + exp((float)-i))) * (1 - (1 / (1 + exp((float)-i))));
}
double TreeActorModel::GetRandValue(double min, double max) const {
    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::uniform_real_distribution<> distrib(min, max);
    return distrib(gen);
}
long double TreeActorModel::mean2ErrDerr(long double predicted,long double trueValue) const{
    return 2*(predicted-trueValue);
}

TreeActorModel::~TreeActorModel() {
    int layerMap[3] = { tokenizedVocap1.size(), EmbedSize, tokenizedVocap1.size() };
    delete[] Data;
    for (int i = 0; i < 2; i++) {
...
long double TreeActorModel::positionalEncoding(int position, int index, int d_model) const {
