...
#ifndef REGRESIF_TRANSFORMER_HPP
#define REGRESIF_TRANSFORMER_HPP
#include "readCSV.hpp"

class RegresifTransformer
{
private:
    DataSet* Data;
    int DataSize, batch_size, EmbedSize, AttentionLayerSize;
...
public:
    RegresifTransformer(const char* DataSetName, int SizeOfData, int data_batch);
    ~RegresifTransformer();
    ...
    void Learn(int epoch, long double learningRate, int max_seq); // Eğitim
};
