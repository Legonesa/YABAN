...
Transformer::Transformer(const char* DataSetName, int SizeOfData, int data_batch, TransformerType type) {
...
void Transformer::Embedding(int embedSize, int epoch, long double learningRate, int attentionSize){
    EmbedSize = embedSize;
...
std::vector<long double> Transformer::predictEmbed(int wordToken, int vocap) const {
...
void Transformer::Learn(int epoch, long double learningRate, int max_seq){
...
std::vector<long double> Transformer::softMax(int outputLenght, long double* Outputs) const {
...
std::vector<long double> Transformer::CrossEntropyLoss(std::vector<long double>& softMaxOutputs, std::vector<long double>& ExpectedValues) const {
...
long double Transformer::cross_entropy_loss(const std::vector<long double>& y_pred, const std::vector<long double>& y_true) const {
...
long double Transformer::sigmoid(long double i) const {
    return 1 / (1 + exp((float)-i));
}
long double Transformer::dSigmoid(long double i) const {
    return (1 / (1 + exp((float)-i))) * (1 - (1 / (1 + exp((float)-i))));
}
double Transformer::GetRandValue(double min, double max) const {
    static std::random_device rd;
    static std::mt19937 gen(rd());
    std::uniform_real_distribution<> distrib(min, max);
    return distrib(gen);
}
long double Transformer::mean2ErrDerr(long double predicted,long double trueValue) const{
...
long double Transformer::positionalEncoding(int position, int index, int d_model) const {
